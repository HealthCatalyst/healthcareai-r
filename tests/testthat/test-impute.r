...
test_that("No recipe with methods trains and predicts.", {
  ...
  expect_equal(res$length[1], 6.27, tolerance = .01)
  expect_equal(as.character(res$color[2]), "Mixed")
  expect_equal(as.character(res$fur[3]), "Short")
  expect_equal(res$width[3], 1.57, tolerance = .01)

  capture_output(res <- impute(d = d_test,
                               animal_id, kitty,
                               recipe = attr(res, "recipe")))
  expect_equal(res$length[1], 6.66, tolerance = .01)
  expect_equal(as.character(res$color[2]), "Mixed")
  expect_equal(as.character(res$fur[3]), "Long")
  expect_equal(res$width[3], 1.95, tolerance = .01)
})
...
test_that("No recipe with methods and params trains and predicts.", {
  ...
  expect_equal(res$length[1], 5.25, tolerance = .01)
  expect_equal(as.character(res$color[2]), "Mixed")
  expect_equal(as.character(res$fur[3]), "Short")
  expect_equal(res$width[3], 1.83, tolerance = .01)

  capture_output(res <- impute(d = d_test,
                               animal_id, kitty,
                               recipe = attr(res, "recipe")))
  expect_equal(res$length[1], 6.6, tolerance = .01)
  expect_equal(as.character(res$color[2]), "Mixed")
  expect_equal(as.character(res$fur[3]), "Short")
  expect_equal(res$width[3], 2.22, tolerance = .01)
})
