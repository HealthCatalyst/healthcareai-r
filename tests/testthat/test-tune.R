...
test_that("tune errors if tune_method isn't 'random'", {
  expect_error(tune(test_df, x3, "regression", tune_method = "grid"),
               "random")
})

test_that("Error informatively if outcome class doesn't match model_class", {
  expect_error(tune(test_df, x1, "regression"), class(test_df$x1))
  test_df$x1 <- as.character(test_df$x1)
  expect_error(tune(test_df, x1, "regression"), class(test_df$x1))
  expect_error(tune(test_df, x3, "classification"), class(test_df$x3))
})
...
test_that("tune errors informatively if the algorithm isn't supported", {
  expect_error(tune(test_df, x3, "regression", "not a model"),
               regexp = "supported")
})
