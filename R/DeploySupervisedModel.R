...
DeploySupervisedModel <- R6Class("DeploySupervisedModel",
  ...
  private = list(
    ...
    #TODO: Set config parameters for building deploy model
    setConfigs = function (p) {
      ...
      if (!is.null(p$type) &&
        ...
        if (self$params$type == 'classification' &&
            IsBinary(self$params$df[[self$params$predictedCol]]) == FALSE) {
          stop('Dependent variable must be binary for classification')
        }
        if (self$params$type == 'regression' &&
            IsBinary(self$params$df[[self$params$predictedCol]]) == TRUE) {
    ...
    loadData = function () {
      ...
      self$params$df <-
        RemoveColsWithAllSameValue(self$params$df)
      ...
      self$params$df <-
        as.data.frame(unclass(self$params$df))
      ...
      if (isTRUE(self$params$impute)) {
        ...
        if (isTRUE(self$params$debug)) {
          print(paste0(
            "Rows in data set before removing rows with NA's: ",
            nrow(self$params$df)
          ))
        }
        ...
        if (isTRUE(self$params$debug)) {
          print(paste0(
            "Rows in data set after removing rows with NA's: ",
            nrow(self$params$df)
          ))
      ...
      trainIndex = createDataPartition(
        y = self$params$df[[self$params$predictedCol]],
        p = 0.8,
        list = FALSE,
        times = 1
      )
