...
**Please note that the package is currently in transition** from an R6 architecture to a tidy, S3 architecture. The code below is the new version, which is in beta and can be acquired via `install.packages("remotes"); remotes::install_github("HealthCatalyst/healthcareai-r")`. Beta testing of the new version, pre-CRAN, is very welcome; please file issues for any bugs, unclear docs, or feature suggestions [here](https://github.com/HealthCatalyst/healthcareai-r/issues). `install.packages("healthcareai")` will get you the old version from CRAN, which will remain available via `remotes::install_github("HealthCatalyst/healthcareai-r@v1.2.4")`.
