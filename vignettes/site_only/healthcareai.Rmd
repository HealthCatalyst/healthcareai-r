...
Let's tune only random forests (by default, xgboost and GLM are also tuned), and to try to really optimize model performance, let's crank `tune_depth` up a little from its default value of ten. That will tune the models over more combinations of hyperparameter values in the search for the best model.
...
You can see how important each "feature" (aka predictor, aka variable) was for making predictions by calling `get_variable_importance` and `plot.variable_importance` on your model object.
...
# Interpret
You can also use GLM to see which features (or categorical levels) are correlated with the outcome by training a GLM model and then
using `interpret` and `plot.interpret` on your model object. These values will be most accurate if you scale your features during data
preparation. That requires using `prep_data` with `scale = TRUE`, then `flash_models` or `tune_models`. In this example, the low value of `weight_class_normal` signifies that people with normal weight are less likely to have diabetes. Similarly, plasma glucose is correlated with having diabetes.

```{r}
glm <- flash_models(d = prepped_training_data,
                    outcome = diabetes,
                    models = "GLM",
                    metric = "PR")
