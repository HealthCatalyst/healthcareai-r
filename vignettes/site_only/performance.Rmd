...
Sample a random subset of data. This keeps half the orginial data with the same
...
These steps ensure you get the best performing model while still being resistent
...
GLM tends to be faster than random forest on long data sets but
sometimes not as accurate. It won't buy you much on very wide data sets. KNN 
...
This chart should help you get an idea of the different algorithms react to
data sets of various sizes.
![ML Speed](figures/speed.png)
...
little more rigor than our svelt `flash_models` but won't take 4 hours.
```{r}
start <- Sys.time()
d_clean <- downsampled_d %>%
  prep_data(outcome = arr_delay, 
